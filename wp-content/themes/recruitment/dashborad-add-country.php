<?php
/*
Template Name: dashboard add country
*/
...
global $wpdb;
$table_name = $wpdb->prefix . 'country_add';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_group'])) {
        ...
        $existing_country = $wpdb->get_row(
            $wpdb->prepare("SELECT id FROM $table_name WHERE LOWER(country_name) = LOWER(%s)", $country_input)
        );
        ...
        if (!empty($selected_country)) {
            ...
            $selected_row = $wpdb->get_row(
                $wpdb->prepare("SELECT id FROM $table_name WHERE country_name = %s", $selected_country)
            );
            ...
            if ($existing_country) {
                ...
                $wpdb->insert($table_name, ['country_name' => $country_input], ['%s']);
...
<div class="groups-header">Countries</div>
<div class="groups-container">
  ...
  <form class="groups-form" method="POST">
    <div class="groups-form-row">
      <div class="groups-form-group">
        <label for="groupSelect" class="groups-label">Select Country</label>
        <select id="groupSelect" name="selected_country" class="groups-select">
          <option value="">-- Select Country --</option>
          <?php
          $countries = $wpdb->get_results("SELECT country_name FROM $table_name ORDER BY country_name ASC");
          if ($countries) {
              foreach ($countries as $c) {
                  echo '<option value="' . esc_attr($c->country_name) . '">' . esc_html($c->country_name) . '</option>';
              }
          }
          ?>
        </select>
      ...
      <div class="groups-form-group">
        <label for="groupName" class="groups-label">Country Name</label>
        <input type="text" id="groupName" name="group_value" class="groups-input" placeholder="Enter country name" autocomplete="off">
      </div>
    </div>
  ...
  <table class="groups-table">
    ...
    <tbody>
      ...
      if ($countries) {
          foreach ($countries as $c) {
              echo '<tr><td>' . esc_html($c->country_name) . '</td></tr>';
          }
      } else {
