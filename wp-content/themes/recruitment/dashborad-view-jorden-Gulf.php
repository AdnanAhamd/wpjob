<?php
/*
Template Name: dashboard-job-view-Gulf
*/
...
// Get search input
// Always define variables safely before use
$title_keyword = isset($_GET['job_title']) ? sanitize_text_field($_GET['job_title']) : '';
// $date_from     = isset($_GET['date_from']) ? sanitize_text_field($_GET['date_from']) : '';
// $date_to       = isset($_GET['date_to']) ? sanitize_text_field($_GET['date_to']) : '';

// Set a default region slug (hardcoded)
$region_slug = 'gulf'; // Or 'jordan' if needed// Only build query if job title is searched
$jobs_query = null;
$jobs_found = false;

if (!empty($title_keyword) || !empty($region_slug)) {
   $args = [
    'post_type'      => 'job',
    'posts_per_page' => -1,
    'post_status'    => 'publish',
    'tax_query'      => [
        [
            'taxonomy' => 'region',
            'field'    => 'slug',      // use 'term_id' if you're using IDs
            'terms'    => 'gulf',      // replace with the actual slug of the term
        ]
    ]
];
...
$jobs_query = new WP_Query($args);
    ...
    //     add_filter('posts_where', function ($where, $query) use ($title_keyword) {
    //         global $wpdb;
    //         if ($query->is_main_query() && $query->get('post_type') === 'job') {
    //             $like = '%' . $wpdb->esc_like($title_keyword) . '%';
